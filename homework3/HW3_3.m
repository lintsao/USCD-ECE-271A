...
for i = 1 : length(alpha)
    ...
    for row = 1 : 255
        for col = 1 : 270
            ...
            fRes = -0.5 * (flatBlock - totalMu1FG(i, :)) * fCovMapInv * transpose(flatBlock - totalMu1FG(i, :)) - ... 
            0.5 * log((2 * pi) ^ 64 * fCovMapDet) + log(priorFG);
    
            bRes = -0.5 * (flatBlock - totalMu1BG(i, :)) * bCovMapInv * transpose(flatBlock - totalMu1BG(i, :)) - ... 
            0.5 * log((2 * pi) ^ 64 * bCovMapDet) + log(priorBG);
    ...
    % Compute error rate;
    allErrorRate3(1, i) = errorCount / (rows * cols);
